...
alpha = 0.01
max_epoch = 10
numMiniBatches = 625 #number of mini-batches
...
batch_samples = int(num_trsamples/numMiniBatches)
...
while epoch <= max_epoch:
    ...
    for b in range(0, num_trsamples, batch_samples):
        
        batch_Xtrain = X_train[b:b+batch_samples]
        batch_ytrain = y_train[b:b+batch_samples]
            
        for i in range (0, batch_samples): #every batch has the same number of images
            
            x = batch_Xtrain[i,:] #input a sample-image = 1 row, 3072 features
            
            x = np.append(x, 1).reshape(-1,1) #epauksimeno, bias +1 in the end 
            
            label = batch_ytrain[i] #correct class {0,...,9}
            #one hot encoding
            d = np.zeros(10)
            d[label] = 1
            d = d.reshape(-1,1)
        ...
        #----update in the end of batch samples with mean of grads----
        for l in range(0,L):
            avg_grad = (1/batch_samples) * sum_grads[l]
            W[l] += alpha * avg_grad
            
            sum_grads[l] = 0 * sum_grads[l] #initialize for the next batch
...
plt.ylabel('Accuracy')
plt.title(f'DNN: L = {L}, hd layers = {L-1}, hd neurons: {N[1:-1]}, learning rate: {alpha}, numOfbatches = {numMiniBatches}\n Accuracy')
plt.legend()
plt.grid(True)
